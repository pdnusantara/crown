...
function ChartTooltip({ active, payload, label, theme }) {
  ...
  return (
    ...
    >
      ...
      <p className="tabular-nums" style={{ color: '#C9A84C' }}>
          ...
          {aggLoading ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chart.days} margin={{ top: 6, right: 6, left: -10, bottom: 0 }}>
                ...
                />
                <Tooltip
                  cursor={{ fill: isLight ? 'rgba(201,168,76,0.08)' : 'rgba(201,168,76,0.06)' }}
                  content={(p) => <ChartTooltip {...p} theme={theme} />}
                />
                <Bar dataKey="commission" radius={[6, 6, 0, 0]} maxBarSize={28}>
                  {chart.days.map((entry, i) => (
                    <Cell key={i} fill={entry.commission === chart.max && chart.max > 0 ? '#E8C875' : '#C9A84C'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
