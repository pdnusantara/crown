    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-5">
      <div>
        <h1 className="font-display text-xl sm:text-2xl font-bold text-off-white flex items-center gap-2">
          <Trophy size={22} className="text-amber-400" /> Peringkat Barber
        </h1>
        <p className="text-muted text-xs sm:text-sm mt-0.5">Berdasarkan omzet bulan ini di cabangmu. Terus semangat! 💪</p>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          <Card>
            <CardBody className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs text-muted">Posisimu</p>
                <p className="text-2xl font-bold text-off-white">
                  #{myRow.rank} <span className="text-sm font-medium text-muted">dari {list.length}</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-brand tabular-nums">{formatRupiah(myRow.revenue)}</p>
                <p className="text-xs text-muted">{myRow.services} layanan{inTop ? ' · di puncak! 🔥' : ''}</p>
              </div>
            </CardBody>
          </Card>
        </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-12 text-muted">
          <AlertCircle size={28} />
          <p className="text-sm">Gagal memuat peringkat.</p>
        </div>
      ) : list.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-12 text-muted text-center">
          ...
          <p className="text-xs">Peringkat akan muncul begitu ada layanan yang selesai.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {list.map((row) => (
