...
export default function BarberQueue() {
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant={item.type === 'booking' ? 'info' : 'muted'}>{t(`queue.ticketType.${item.type}`)}</Badge>
                  </div>
                  ...
                  {item.status === 'waiting' && (
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-xs text-amber-400">{t('queue.waitingTime', { n: item.waitTime })}</span>
                    </div>
                  )}
                </div>
                {(item.status === 'waiting' || item.status === 'in-progress') && (
                  ...
                  >
                    {item.status === 'waiting' ? t('queue.start') : t('queue.finish')}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-display text-2xl font-bold text-off-white">{t('queue.myQueue')}</h1>
            <LiveBadge />
          </div>
          <p className="text-muted text-sm mt-1">{t('queue.summary', { inProgress: inProgress.length, waiting: waiting.length })}</p>
        </div>
        <button
          type="button"
          onClick={toggleMuted}
          aria-label={muted ? t('barber.queueAlertUnmute') : t('barber.queueAlertMute')}
          title={muted ? t('barber.queueAlertMutedTitle') : t('barber.queueAlertOnTitle')}
      ...
      <Section title={t('queue.inProgressShort')} items={inProgress} color="bg-blue-400" />
      <Section title={t('queue.waiting')} items={waiting} color="bg-amber-400" />
      <Section title={t('queue.doneToday')} items={done} color="bg-green-400" />
    </div>
  )
}
